...
LocaleConfig.locales['zh'] = {
  ...
  dayNames: ['星期日','星期開一','星期二','星期三','星期四','星期五','星期六'],
...
// --- 1. 即時測量分頁 (HomeScreen) ---
const HomeScreen = ({ user, bp, setBp, handleOCR, autoSaveRecord, lastRecord, onSync }: any) => {
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView style={styles.inputArea} keyboardShouldPersistTaps="handled">
        ...
        {/* 核心同步功能 */}
        <TouchableOpacity style={[styles.ocrBtn, {backgroundColor: '#52c41a'}]} onPress={() => onSync(true)}>
          <Text style={styles.saveBtnText}>🔄 從 Health Connect 同步 (OMRON)</Text>
        </TouchableOpacity>
        ...
        <TouchableOpacity style={styles.saveBtn} onPress={() => autoSaveRecord(bp)}><Text style={styles.saveBtnText}>💾 手動儲存數據</Text></TouchableOpacity>
      </ScrollView>
    </SafeAreaView>
  );
};
...
const DiaryScreen = ({ records, selectedDate, setSelectedDate, exportToCSV }: any) => {
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.diaryDetailHeader}><Text style={styles.detailTitle}>📋 {selectedDate} 紀錄</Text><TouchableOpacity onPress={exportToCSV} style={styles.miniBtn}><Text style={{color:'#fff', fontSize:12}}>匯出 CSV</Text></TouchableOpacity></View>
...
const App = () => {
  ...
  const syncHealthData = async (isManual = false) => {
    try {
      ...
      if (!isInitialized) return isManual && Alert.alert("提示", "裝置不支援 Health Connect");
      
      await requestPermission([{ accessType: 'read', recordType: 'BloodPressure' }]);
      
      const curTime = new Date();
      const startTime = new Date(curTime.getTime() - 30 * 24 * 60 * 60 * 1000); 
      const result = await readRecords('BloodPressure', {
        timeRangeFilter: { operator: 'between', startTime: startTime.toISOString(), endTime: curTime.toISOString() }
      });

      if (result.records.length === 0) return isManual && Alert.alert("提示", "無新紀錄");

      const newSyncRecords = result.records.map((record: any) => {
        const rDate = new Date(record.time);
        const timeStr = `${rDate.getFullYear()}/${(rDate.getMonth()+1).toString().padStart(2,'0')}/${rDate.getDate().toString().padStart(2,'0')} ${rDate.toLocaleTimeString('zh-TW', { hour12: false })}`;
        const s = record.systolic.value;
        const d = record.diastolic.value;
        ...
        return { 
          ...
          level, 
          time: timeStr, 
          id: record.metadata?.id || (Date.now() + Math.random()).toString(), 
          userId: user?.uid 
        };
      });

      const finalToSave = newSyncRecords.filter(nr => !records.some(r => r.time === nr.time));
      
      if (finalToSave.length > 0) {
        const updated = [...finalToSave, ...records];
        setRecords(updated); 
        analyzeHealth(updated);
        await AsyncStorage.setItem(`bp_records_${user?.uid}`, JSON.stringify(updated));
        for (const rec of finalToSave) { try { await axios.post(API_URL, rec); } catch (e) {} }
        Alert.alert("同步完成", `已匯入 ${finalToSave.length} 筆新數據`);
      } else if (isManual) { 
        Alert.alert("提示", "數據已是最新"); 
      }
    } catch (e) { 
      if (isManual) Alert.alert("錯誤", "同步失敗，請檢查系統授權設定"); 
    }
  };
  ...
  useEffect(() => {
    if (user) {
      const timer = setTimeout(() => syncHealthData(false), 2000);
  ...
  const autoSaveRecord = async (data: any) => {
    ...
    const newRecord = { sys: s.toString(), dia: d.toString(), pulse: data.pulse || '75', level, time: timeStr, id: Date.now().toString(), userId: user?.uid };
    const updated = [newRecord, ...records];
    setRecords(updated); analyzeHealth(updated); setBp({ sys: '', dia: '', pulse: '' });
    try { 
      await AsyncStorage.setItem(`bp_records_${user?.uid}`, JSON.stringify(updated)); 
  ...
  const handleOCR = async () => {
    ...
    if (result.assets) {
      ...
      if (nums && nums.length >= 2) setBp({ ...bp, sys: nums[0], dia: nums[1] });
    }
  };
  ...
  if (!user) return (
    <SafeAreaView style={styles.loginContainer}>
      ...
      <TouchableOpacity style={styles.loginBtn} onPress={() => auth().signInWithEmailAndPassword(email, password)}><Text style={styles.btnText}>立即登入</Text></TouchableOpacity>
      <TouchableOpacity style={[styles.loginBtn, {backgroundColor: '#52c41a'}]} onPress={() => auth().createUserWithEmailAndPassword(email, password)}><Text style={styles.btnText}>註冊新帳號</Text></TouchableOpacity>
    </SafeAreaView>
  );
  ...
  return (
    <NavigationContainer>
      <Tab.Navigator screenOptions={{ tabBarStyle: { height: 70, paddingBottom: 12, backgroundColor: '#fff', elevation: 15 }, tabBarIconStyle: { display: 'none' }, tabBarActiveTintColor: '#154360', headerRight: () => (
        <TouchableOpacity onPress={() => auth().signOut()} style={{ marginRight: 15 }}><Text style={{ color: '#ff4d4f', fontWeight: 'bold' }}>登出</Text></TouchableOpacity>
      )}}>
        <Tab.Screen name="Measure" options={{ title: '📸 測量' }}>
          {() => <HomeScreen user={user} bp={bp} setBp={setBp} handleOCR={handleOCR} autoSaveRecord={autoSaveRecord} lastRecord={records[0]} onSync={(manual: boolean) => syncHealthData(manual)} />}
        </Tab.Screen>
...
const styles = StyleSheet.create({
  ...
  input: { flex: 0.48, backgroundColor: 'white', padding: 15, borderRadius: 10, borderWidth: 1, borderColor: '#ddd' },
  ocrBtn: { backgroundColor: '#cf1322', padding: 18, borderRadius: 12, alignItems: 'center', marginBottom: 15 },
