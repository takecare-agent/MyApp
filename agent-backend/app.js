...
app.post('/bind', async (req, res) => {
    ...
    try {
        ...
        if (target.role === 'caregiver' || me.role === 'caregiver') {
            me.boundTo = target.username; me.boundToName = target.name;
            target.boundTo = me.username; target.boundToName = me.name;
        } else if (target.role === 'care_recipient' || me.role === 'care_recipient') {
            me.patientBoundTo = target.username; me.patientBoundToName = target.name;
            target.patientBoundTo = me.username; target.patientBoundToName = me.name;
        }
...
app.post('/unbind', async (req, res) => {
    const { username, type } = req.body; // type: 'caregiver' or 'patient'
    try {
        const me = await User.findOne({ username });
        if (type === 'caregiver' && me.boundTo) {
            const partner = await User.findOne({ username: me.boundTo });
            if (partner) { partner.boundTo = null; partner.boundToName = null; await partner.save(); }
            me.boundTo = null; me.boundToName = null;
        } else if (type === 'patient' && me.patientBoundTo) {
            const partner = await User.findOne({ username: me.patientBoundTo });
            if (partner) { partner.patientBoundTo = null; partner.patientBoundToName = null; await partner.save(); }
            me.patientBoundTo = null; me.patientBoundToName = null;
        }
        await me.save();
        res.json({ success: true });
    } catch (e) { res.json({ success: false }); }
...
app.get('/logs', async (req, res) => {
    try {
        ...
        const logs = await Log.find({ senderUsername: { $in: targets }, isDangerous: true }).sort({ timestamp: -1 }).limit(20);
