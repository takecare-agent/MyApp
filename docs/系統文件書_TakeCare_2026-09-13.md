...
2. MediaPipe 估姿勢；TFLite 輸出 fall 分數。  
3. 髖夠低且符合躺姿才確認摔倒；持續蹲／彎不得因 5 秒變摔倒。  
4. 蹲下／彎腰：寫低嚴重度紀錄＋截圖，不推播。  
5. 確認摔倒：錄短片；未起 ≥5 秒 → 高；持續未起另推極高波次（2／5／15／35 分）。  
6. 後端寫事件、推播看護與家屬。  
7. 看護一鍵已查看；家屬只看。  
...
- 模型側：Lab 混淆矩陣、thresholds_v8（研究資料在 `Fall_Detection_Lab`）。  
- 產品側：真機兩台驗收（華為安卓＋iPhone），不以三台模擬器為預設。  
- 歐姆龍同步：2026-09-12 用戶口述成功。  
- 六語：字典掃描非中文 0 筆繁中混入；截斷已解急救／日誌卡。  
- 限制：未做大規模臨床試驗；跌倒測試用緩慢躺下、禁止真摔。
