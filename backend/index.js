...
const reminderSchema = new mongoose.Schema({
  ...
  recurringWeekdays: [{ type: Number, min: 0, max: 6 }]
}, { timestamps: true })
...
app.get("/family/care-records/history", async (req, res) => {
  ...
  const user = await User.findOne({ email: decoded.email })
  const patientUserId = await resolveTargetPatient(user._id)
  console.log(`[Family Care Records] user=${user.email}, patientUserId=${patientUserId}`)

  const [familyRecords, caregiverLogs] = await Promise.all([
    FamilyCareRecord.find({ userId: user._id }).sort({ recordDate: -1, _id: -1 }).limit(50),
    CaregiverCareLog.find({ patientUserId }).sort({ happenedAt: -1, _id: -1 }).limit(50)
  ])
  console.log(`[Family Care Records] familyRecords=${familyRecords.length}, caregiverLogs=${caregiverLogs.length}`)

  const caregiverUserIds = [...new Set(caregiverLogs.map(l => l.userId.toString()))]
  const caregiverUsers = await User.find({ _id: { $in: caregiverUserIds } }).select("name displayName")
  const caregiverNameMap = Object.fromEntries(caregiverUsers.map(u => [u._id.toString(), u.displayName || u.name || "照護員"]))

  const merged = [
    ...familyRecords.map(r => ({ ...r.toObject(), _id: `family-${r._id}`, recordDate: r.recordDate })),
    ...caregiverLogs.map(l => ({
      ...l.toObject(),
      _id: `caregiver-${l._id}`,
      recordDate: l.happenedAt,
      caregiverName: caregiverNameMap[l.userId.toString()] || "照護員",
      isCaregiverLog: true
    }))
  ].sort((a, b) => {
    const dateA = new Date(a.recordDate || a.happenedAt)
    const dateB = new Date(b.recordDate || b.happenedAt)
    return dateB - dateA
  })
...
// Family Care Records CRUD
app.post("/family/care-records", async (req, res) => {
  const decoded = verifyToken(req)
  if (!decoded) return res.status(401).json({ message: "Invalid token" })
  const user = await User.findOne({ email: decoded.email })
...
app.put("/family/care-records/:id", async (req, res) => {
  const decoded = verifyToken(req)
  if (!decoded) return res.status(401).json({ message: "Invalid token" })
  const user = await User.findOne({ email: decoded.email })
  console.log("PUT /family/care-records/:id body:", req.body)
  const updateData = {}
  if (req.body.recordDate) updateData.recordDate = new Date(req.body.recordDate)
  if (req.body.category !== undefined) updateData.category = req.body.category
  if (req.body.subItem !== undefined) updateData.subItem = req.body.subItem
  if (req.body.note !== undefined) updateData.note = req.body.note
  console.log("Updating with:", updateData)
  const record = await FamilyCareRecord.findOneAndUpdate(
...
app.delete("/family/care-records/:id", async (req, res) => {
  const decoded = verifyToken(req)
  if (!decoded) return res.status(401).json({ message: "Invalid token" })
  const user = await User.findOne({ email: decoded.email })
  await FamilyCareRecord.findOneAndDelete({ _id: req.params.id, userId: user._id })
  res.json({ message: "Deleted" })
})

// Caregiver Care Logs CRUD
app.post("/caregiver/care-logs", async (req, res) => {
  const decoded = verifyToken(req)
  if (!decoded) return res.status(401).json({ message: "Invalid token" })
  const user = await User.findOne({ email: decoded.email })
  const patientUserId = await resolveTargetPatient(user._id)
  console.log(`[Caregiver Create Log] user=${user.email}, patientUserId=${patientUserId}`)
  const record = await CaregiverCareLog.create({
    ...
    happenedAt: req.body.recordDate ? new Date(req.body.recordDate) : new Date(),
    category: req.body.category,
    subItem: req.body.subItem,
    note: req.body.note
  })
...
app.put("/caregiver/care-logs/:id", async (req, res) => {
  const decoded = verifyToken(req)
  if (!decoded) return res.status(401).json({ message: "Invalid token" })
  const user = await User.findOne({ email: decoded.email })
  console.log("PUT /caregiver/care-logs/:id body:", req.body)
  const updateData = {}
  if (req.body.recordDate) updateData.happenedAt = new Date(req.body.recordDate)
  if (req.body.tasks !== undefined) updateData.tasks = req.body.tasks
  if (req.body.vitals !== undefined) updateData.vitals = req.body.vitals
  if (req.body.category !== undefined) updateData.category = req.body.category
  if (req.body.subItem !== undefined) updateData.subItem = req.body.subItem
  if (req.body.note !== undefined) updateData.note = req.body.note
  console.log("Updating with:", updateData)
  const record = await CaregiverCareLog.findOneAndUpdate(
    { _id: req.params.id, userId: user._id },
    { $set: updateData },
    { new: true }
  )
...
app.delete("/caregiver/care-logs/:id", async (req, res) => {
  const decoded = verifyToken(req)
  if (!decoded) return res.status(401).json({ message: "Invalid token" })
  const user = await User.findOne({ email: decoded.email })
  await CaregiverCareLog.findOneAndDelete({ _id: req.params.id, userId: user._id })
  res.json({ message: "Deleted" })
})
...
app.post("/family/reminders", async (req, res) => {
  ...
  if (!decoded) return res.status(401).json({ message: "Invalid token" })
  const user = await User.findOne({ email: decoded.email })
  const { category, content, time, note, isRecurring, recurringWeekdays } = req.body || {}
  ...
  const record = await Reminder.create({
    ...
    recurringWeekdays: Array.isArray(recurringWeekdays) ? recurringWeekdays : []
  })
...
app.patch("/family/reminders/:id", async (req, res) => {
  ...
  if (typeof isRecurring === "boolean") record.isRecurring = isRecurring
  if (Array.isArray(recurringWeekdays)) record.recurringWeekdays = recurringWeekdays
  await record.save()
  res.json({ message: "OK", record })
})
...
app.post("/caregiver/care-logs/sync", async (req, res) => {
  ...
  const record = await CaregiverCareLog.create({ userId: user._id, patientUserId, logId: sample.logId, tasks: sample.tasks, vitals: sample.vitals, happenedAt: new Date(sample.happenedAt), source: "mock-seed" })
